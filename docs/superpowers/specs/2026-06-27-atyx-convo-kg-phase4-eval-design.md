...
`add_noise.py`'s **SNR control is unchanged** — only the noise *source* differs from the original synthetic-pink plan. SNR remains the single controlled variable. The clip is shorter (~128 s) than the 160 s slice; `add_noise.fit_noise` tiles it seamlessly to length before mixing, so coverage is complete.
