...
7. **Scope:** single-hop is the v1 hero path and **generalizes genuinely** on the local ~9B model — given only live, data-driven schema grounding (relationship-direction + entity-name samples, label set, property-graph rules), the model writes correct single-hop Cypher across rephrasings, with real edge-level provenance. The graph is multi-hop-ready, and the showcase chain **exists structurally and was verified** against the live induced sample2 graph: `(business ownership :AssetClass)-[:HAS_STRATEGY]->(Do your own business :WealthStrategy)-[:ACHIEVES_GOAL]->(get rich before 30 :FinancialGoal)` (asset → strategy → goal, 2 hops, grounded in `stmt:sample2:0`). **However — measured 2026-06-26 — the local ~9B model cannot reliably navigate this 2-hop chain via text-to-Cypher.** An earlier multi-hop test passed only because the demo answer was hardcoded into the prompt; that overfitting was caught and removed (the honest no-answer-smuggling test: *would this prompt text be equally correct for any other induced graph?*). With **generic, data-driven 2-hop path-pattern sampling** added, the model reached only **~3/5 across rephrasings** — below the "robust, not rehearsed" bar — so that code was reverted rather than shipped as a brittle demo. Root cause of the residual failures is **entity-resolution ambiguity**, not path-pattern ignorance: phrasings like "starting/running your own business" resolve to the `WealthStrategy` node and bypass the first hop. The multi-hop acceptance test is marked `xfail(strict=False)` with this finding. Crossing multi-hop reliably needs entity linking, guided query decomposition, or a stronger model — see the design note's limitations section. This is reported as a **capability boundary, not a failure**: catching and removing the overfit, then honestly measuring the boundary, is the deliverable.
